...
func MiddleWare(c *gin.Context) {
	...
	// 中间件开始执行
	// handler共消耗时间： 2.00028165s
	// 中间件+handler共消耗时间： 2.000404446s
	// 获取了中间件中写入到 gin.Context 中的值： 这是一个中间件样例 XD
	//
	// 如果写了 c.Next() ，那么会是这种输出结果：
	// 中间件开始执行
	// handler共消耗时间： 2.000796193s
	// 获取了中间件中写入到 gin.Context 中的值： 这是一个中间件样例 XD
	// 中间件+handler共消耗时间： 5.001351866s
	//
...
func main() {
	...
	// r.Use() 将参数中定义的中间件附加到 router 中为注册中间件，通过该方法附加的中间件将包含在每个请求的处理程序中
	// 后面的代码逻辑，都会先执行 MiddleWare() 这个中间件
	r.Use(MiddleWare)
